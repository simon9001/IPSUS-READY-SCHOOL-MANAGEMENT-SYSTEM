...
export const examsService = {
  ...
  async getScaleById(id: number) {
    const scale = await examsRepository.findScaleById(id)
    if (!scale) throw new NotFoundError(`Grading scale ${id} not found`)
    const bands = await examsRepository.findBandsForScale(id)
    return { ...scale, bands }
  },

  createScale: (input: CreateGradingScaleInput) => {
    const { bands, ...scale } = input
    return examsRepository.createScale(
      scale,
      bands.map((b) => ({ minMarks: String(b.minMarks), maxMarks: String(b.maxMarks), grade: b.grade, points: b.points !== undefined ? String(b.points) : undefined })),
    )
  },
  ...
  async recordResult(input: RecordExamResultInput) {
  ...
  async bulkRecordResults(results: RecordExamResultInput[]) {
    const recorded = []
  ...
  async reportCard(examId: number, studentId: number): Promise<ReportCard> {
    ...
    if (!exam) throw new NotFoundError(`Exam ${examId} not found`)

    const rows = await examsRepository.findResultsByExamAndStudent(examId, studentId)
    if (rows.length === 0) throw new NotFoundError(`No results recorded for student ${studentId} in exam ${examId}`)
    ...
    const totalsByStudent = new Map<number, number>()
    for (const r of allResults) totalsByStudent.set(r.studentId, (totalsByStudent.get(r.studentId) ?? 0) + Number(r.marks))
