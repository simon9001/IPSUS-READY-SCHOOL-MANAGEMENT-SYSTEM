...
export const bankingService = {
  ...
  async getAccountById(id: number) {
    ...
    if (!account) throw new NotFoundError(`Bank account ${id} not found`)
    return account
  },

  createAccount: (input: CreateBankAccountInput) => bankingRepository.createAccount(input),

  listReconciliations: (bankAccountId: number) => bankingRepository.findReconciliationsByAccount(bankAccountId),

  createReconciliation: (input: CreateReconciliationInput) => {
    const { items, ...reconciliation } = input
    return bankingRepository.createReconciliation(
      { ...reconciliation, statementBalance: String(reconciliation.statementBalance), bookBalance: String(reconciliation.bookBalance) },
      items.map((item) => ({ ...item, amount: String(item.amount) })),
    )
  },

  markReconciled: (id: number, reconciledBy: number) => bankingRepository.markReconciled(id, reconciledBy),

  listImprestRequests: () => bankingRepository.findAllImprestRequests(),

  async getImprestRequestById(id: number) {
    const request = await bankingRepository.findImprestRequestById(id)
    if (!request) throw new NotFoundError(`Imprest request ${id} not found`)
    return request
  },

  async issueImprest(input: IssueImprestInput) {
  ...
  async retireImprest(imprestRequestId: number, input: RetireImprestInput) {
    const request = await bankingRepository.findImprestRequestById(imprestRequestId)
    if (!request) throw new NotFoundError(`Imprest request ${imprestRequestId} not found`)
    if (request.status !== 'issued') throw new ConflictError(`Imprest request ${imprestRequestId} is not in 'issued' status`)
