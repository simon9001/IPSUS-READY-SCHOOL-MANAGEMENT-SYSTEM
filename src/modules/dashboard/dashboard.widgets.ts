...
interface WidgetDef {
  ...
  build: (ctx: WidgetContext) => Promise<DashboardWidget>
}

const hasGate = (permissions: string[], gate: string | string[]) =>
  Array.isArray(gate) ? gate.some((p) => permissions.includes(p)) : permissions.includes(gate)
...
export const WIDGETS: WidgetDef[] = [
  ...
  },
  {
    ...
    async build() {
      ...
      for (const inv of invoices) byStatus.set(inv.status, (byStatus.get(inv.status) ?? 0) + 1)
      return {
        id: 'fees-overview',
        title: 'Fees',
        kind: 'stats',
  ...
  },
  {
    ...
    async build() {
      ...
      const byStatus = new Map<string, number>()
      for (const b of budgets) byStatus.set(b.status, (byStatus.get(b.status) ?? 0) + 1)
      return {
        id: 'budget-overview',
        title: 'Budgets',
        kind: 'stats',
  ...
  },
  {
    ...
    async build({ permissions }) {
      const rows: { label: string; sublabel?: string; value?: string; tone?: 'warning' }[] = []
  ...
  },
  {
    ...
    async build() {
      ...
      const rows: { label: string; sublabel?: string; tone?: 'warning' | 'danger' }[] = []
  ...
  // ---------------- STUDENTS & ACADEMIC ----------------
  {
    ...
    async build() {
      ...
      for (const s of active) byClass.set(s.classId, (byClass.get(s.classId) ?? 0) + 1)
      return {
        id: 'enrollment',
        title: 'Enrollment',
        kind: 'stats',
  ...
  },
  {
    ...
    async build() {
      ...
      let mostRecent: { name: string; mean: number } | null = null
  ...
  },
  {
    ...
    async build() {
      ...
      const byStatus = new Map<string, number>()
      for (const a of admissions) byStatus.set(a.status, (byStatus.get(a.status) ?? 0) + 1)
      return {
        id: 'admissions-pipeline',
  ...
  },
  {
    ...
    async build({ asOfDate }) {
      ...
      const expiring = contracts.filter(
        (c) => c.status === 'active' && c.endDate && daysUntil(asOfDate, c.endDate) >= 0 && daysUntil(asOfDate, c.endDate) <= 60,
      )
      return {
        id: 'contracts-expiring',
...
export const SECTION_ORDER: DashboardSectionId[] = ['attention', 'system', 'financial', 'students', 'hr', 'welfare', 'compliance', 'general']
