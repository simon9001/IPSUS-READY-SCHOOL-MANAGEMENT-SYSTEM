...
async function studentName(studentId: number): Promise<string> {
...
export const disciplinaryCasesService = {
  ...
  listByStudent: (studentId: number) => disciplinaryCasesRepository.findByStudent(studentId),

  async getById(id: number) {
    const record = await disciplinaryCasesRepository.findById(id)
    if (!record) throw new NotFoundError(`Disciplinary case ${id} not found`)
    return record
  },
