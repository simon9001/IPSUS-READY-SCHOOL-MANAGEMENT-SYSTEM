...
import type { CreateLeaveRequestInput, CreateLeaveTypeInput } from './leave.schema.js'
import type { LeaveBalance } from './leave.types.js'

function inclusiveDayCount(startDate: string, endDate: string): number {
  const start = new Date(startDate)
  const end = new Date(endDate)
  const days = Math.round((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)) + 1
  if (days <= 0) throw new ValidationError('endDate must be on or after startDate')
  return days
}

export const leaveService = {
  listTypes: () => leaveRepository.findAllTypes(),
  createType: (input: CreateLeaveTypeInput) => leaveRepository.createType(input),

  list: () => leaveRepository.findAll(),
  listByStaff: (staffId: number) => leaveRepository.findByStaff(staffId),
  ...
  async balance(staffId: number, leaveTypeId: number, year: number): Promise<LeaveBalance> {
    const leaveType = await leaveRepository.findTypeById(leaveTypeId)
    if (!leaveType) throw new NotFoundError(`Leave type ${leaveTypeId} not found`)
  ...
  async apply(input: CreateLeaveRequestInput) {
  ...
  async approve(id: number, approverId: number) {
    const request = await leaveRepository.findById(id)
    if (!request) throw new NotFoundError(`Leave request ${id} not found`)
    if (request.status !== 'pending') throw new ConflictError(`Leave request ${id} is ${request.status}, not pending`)
