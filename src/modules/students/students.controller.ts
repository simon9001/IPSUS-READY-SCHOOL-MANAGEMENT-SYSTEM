...
export const studentsController = {
  ...
  createClass: async (c: Context) =>
    created(c, await studentsService.createClass(getValidated<CreateClassInput>(c, 'json'), actorId(c))),
  ...
  createStream: async (c: Context) =>
    created(c, await studentsService.createStream(getValidated<CreateStreamInput>(c, 'json'), actorId(c))),
  ...
  create: async (c: Context) =>
    created(c, await studentsService.create(getValidated<CreateStudentInput>(c, 'json'), actorId(c))),
  update: async (c: Context) =>
    ok(c, await studentsService.update(Number(c.req.param('id')), getValidated<UpdateStudentInput>(c, 'json'), actorId(c))),
  remove: async (c: Context) => ok(c, await studentsService.remove(Number(c.req.param('id')), actorId(c))),
}
