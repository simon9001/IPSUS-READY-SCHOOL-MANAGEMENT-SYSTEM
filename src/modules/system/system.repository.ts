...
import type { RoleUsage, UserAccountStats } from './system.types.js'
...
export const systemRepository = {
  ...
  async userStats(): Promise<UserAccountStats> {
    const [row] = await db
      .select({
        total: count(),
        ...
        locked: sql<number>`count(*) filter (where ${users.status} = 'locked')`.mapWith(Number),
        lockedOut: sql<number>`count(*) filter (where ${users.lockedUntil} > now())`.mapWith(Number),
        mustChangePassword: sql<number>`count(*) filter (where ${users.mustChangePassword})`.mapWith(Number),
        neverLoggedIn: sql<number>`count(*) filter (where ${users.lastLoginAt} is null)`.mapWith(Number),
        withFailedAttempts: sql<number>`count(*) filter (where ${users.failedLoginAttempts} > 0)`.mapWith(Number),
      })
      .from(users)
    return row
  },
