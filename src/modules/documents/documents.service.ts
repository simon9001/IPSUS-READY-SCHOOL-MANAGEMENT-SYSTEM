...
import type { Transcript } from './documents.types.js'

function nextReferenceNumber(): string {
  return `DOC-${Date.now()}`
}

async function studentFullName(studentId: number): Promise<{ name: string; admissionNo: string }> {
...
export const documentsService = {
  ...
  createTemplate: (input: CreateTemplateInput) => documentsRepository.createTemplate(input),

  listAll: () => documentsRepository.findAllDocuments(),
  listByStudent: (studentId: number) => documentsRepository.findByStudent(studentId),
  ...
  async renderLetter(input: RenderLetterInput) {
    const template = await documentsRepository.findTemplateByCode(input.templateCode)
    if (!template) throw new NotFoundError(`Document template "${input.templateCode}" not found`)
    if (!template.isActive) throw new ValidationError(`Document template "${input.templateCode}" is inactive`)
  ...
  async generateTranscript(input: GenerateTranscriptInput) {
    const { name, admissionNo } = await studentFullName(input.studentId)
    const rows = await examsService.getAllResultsForStudent(input.studentId)
    if (rows.length === 0) throw new ValidationError(`Student ${input.studentId} has no exam results to compile a transcript from`)
  ...
  async generateFeeClearanceLetter(input: GenerateFeeClearanceInput) {
    const { name, admissionNo } = await studentFullName(input.studentId)
