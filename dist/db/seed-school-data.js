...
const skipped = [];
async function tolerate(label, work) {
    try {
        return await work();
    }
    catch (err) {
...
function schoolDays(start, count) {
    const days = [];
...
async function main() {
    ...
    const acct = (code) => {
        ...
        if (!row)
            throw new Error(`Account ${code} not found — is the chart of accounts seeded?`);
        return row.id;
    };
    const fund = (code) => {
    ...
    const period = (term) => {
        ...
        if (!row)
            throw new Error(`2026 Term ${term} not found`);
        return row.id;
    };
    ...
    const actor = (email) => userIdByEmail.get(email) ?? adminId;
    ...
    for (const cls of classes) {
        for (let i = 0; i < 14; i++) {
            ...
            // both existing.
            const boarding = chance(0.65) ? 'boarder' : 'day';
    ...
    const teacherByStaffNo = new Map();
    ...
    const subjectsForClass = (level) => (level <= 2 ? JUNIOR_SUBJECTS : SENIOR_SUBJECTS);
    ...
    const teacherFor = (code, level) => {
        const candidates = teachersBySubject.get(code);
        if (!candidates || candidates.length === 0)
            return teacherByStaffNo.get(TEACHERS[0].staffNo);
    ...
    const feeStructureFor = new Map();
    for (const cls of classes) {
        for (const boarding of ['day', 'boarder']) {
            for (const [term, periodId] of [[1, P1], [2, P2], [3, P3]]) {
                const items = [
    ...
    for (const [term, periodId, invoiceDate] of [[1, P1, TERM_1.start], [2, P2, TERM_2.start], [3, P3, TERM_3.start]]) {
        for (const student of allStudents) {
            ...
            const method = pick(['mpesa', 'bank', 'cash', 'cheque']);
            const reference = `REF${randInt(100000, 999999)}`;
            const invoice = await tolerate(`Term ${term} invoice for ${student.name}`, () => feesService.createInvoice({
            ...
            const payment = await tolerate(`Term ${term} receipt for ${student.name}`, () => feesService.recordPayment({
                studentId: student.id,
    ...
    for (const student of allStudents) {
        for (let i = 0, target = randInt(1, 5); i < target; i++) {
            ...
            const awarded = await tolerate(`conduct points for ${student.name}`, () => conductPointsService.award({ studentId: student.id, periodId, ruleId, awardedBy: TEACHER_USER }));
            if (awarded)
                awards++;
        }
    }
    const incidents = [
        { severity: 'minor', description: 'Found outside the classroom during a lesson without permission.', actionTaken: 'Verbal warning issued and the class teacher informed.' },
        { severity: 'moderate', description: 'Repeated noise-making during evening preps despite prior warnings.', actionTaken: 'Two days of supervised manual work; parent contacted by phone.' },
        { severity: 'minor', description: 'Reported to the parade in an incomplete school uniform.', actionTaken: 'Uniform corrected the same day; recorded for monitoring.' },
        { severity: 'major', description: 'Left the school compound without a gate pass during afternoon lessons.', actionTaken: 'Parent summoned; referred to the Deputy Principal for a formal hearing.' },
        { severity: 'moderate', description: 'Damage to a laboratory stool during a practical session.', actionTaken: 'Restitution agreed with the parent; counselled on laboratory conduct.' },
    ];
    ...
    const bedCounters = new Map(dormitories.map((d) => [d.id, 1]));
    const allocatedBoarders = [];
    ...
    for (const night of schoolDays(TERM_3.start, 3)) {
        const records = allocatedBoarders.map((student) => ({
            ...
            status: (chance(0.96) ? 'present' : chance(0.5) ? 'absent' : 'on_leave'),
    ...
    const conditions = [
        { condition: 'Asthma', severity: 'moderate', notes: 'Carries a salbutamol inhaler; avoid dusty duties.' },
        { condition: 'Peanut allergy', severity: 'severe', notes: 'Kitchen and matron notified. Antihistamines held at the clinic.' },
        { condition: 'Sickle cell trait', severity: 'mild', notes: 'Ensure adequate hydration during games.' },
        { condition: 'Epilepsy', severity: 'moderate', notes: 'On regular medication; dormitory captain briefed.' },
    ];
    ...
    const clubPlan = [
        { name: 'Drama Club', category: 'club', patron: 'TSC/002' },
        { name: 'Science Congress', category: 'society', patron: 'TSC/005' },
        { name: 'Football (Boys)', category: 'sport', patron: 'TSC/001' },
        { name: 'Volleyball (Girls)', category: 'sport', patron: 'TSC/004' },
        { name: 'Christian Union', category: 'society', patron: 'TSC/006' },
        { name: 'Debate & Journalism Club', category: 'club', patron: 'BOM/T03' },
    ];
    const clubs = [];
    ...
    const competitionPlan = [
        { name: 'Sub-County Drama Festival', clubIndex: 0, level: 'zonal', date: '2026-05-20', venue: 'Dagoretti High School' },
        { name: 'County Science Congress', clubIndex: 1, level: 'county', date: '2026-06-17', venue: 'Nairobi School' },
        { name: 'Regional Term 2 Ball Games', clubIndex: 2, level: 'regional', date: '2026-07-08', venue: 'Nyayo National Stadium' },
        { name: 'National Drama Festival', clubIndex: 0, level: 'national', date: '2026-04-14', venue: 'Nakuru' },
    ];
    let participants = 0;
    ...
    console.log('Notices...');
    const noticePlan = [
        { title: 'Term 3 Opening — 31 August 2026', body: 'All learners report on Monday 31 August 2026 by 4.00pm. Boarders should carry the full list of personal effects circulated last term. Fee balances for Term 2 must be cleared before reporting.', audience: 'all' },
        { title: 'Form 4 KCSE Registration', body: 'Form 4 candidates must confirm their KCSE registration details at the Deputy Principal’s office before Friday 11 September 2026. Corrections cannot be made after the KNEC deadline.', audience: 'all' },
        { title: 'Parents’ Academic Day — 26 September 2026', body: 'Parents and guardians are invited to the Term 3 Academic Day on Saturday 26 September 2026 from 9.00am. Class teachers will issue Term 2 report forms and discuss individual progress.', audience: 'parents' },
        { title: 'Staff Briefing — Monday 7 September', body: 'All teaching staff to attend a briefing in the staffroom at 4.15pm on the Term 3 examination calendar and syllabus coverage returns.', audience: 'staff' },
        { title: 'Water Rationing Advisory', body: 'Following the county water supply interruption, dormitories will have running water between 5.00am–8.00am and 5.00pm–9.00pm until further notice.', audience: 'all' },
    ];
    ...
    if (skipped.length > 0) {
        ...
        for (const entry of skipped.slice(0, 15))
            console.log(`  - ${entry}`);
