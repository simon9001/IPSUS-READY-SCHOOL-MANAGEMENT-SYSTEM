...
export const portalService = {
    myChildren: (guardianUserId) => guardiansService.listStudentsForGuardian(guardianUserId),
    async feeStatement(guardianUserId, studentId) {
    ...
    async reportCard(guardianUserId, studentId, examId) {
        await guardiansService.assertGuardianOfStudent(guardianUserId, studentId);
        return examsService.reportCard(examId, studentId);
    },
    async attendance(guardianUserId, studentId) {
        await guardiansService.assertGuardianOfStudent(guardianUserId, studentId);
        return attendanceService.listByStudent(studentId);
    },
    async notices(guardianUserId) {
        ...
        const byId = new Map();
        for (const notice of await noticesService.listForParents())
            byId.set(notice.id, notice);
        for (const classId of classIds) {
            for (const notice of await noticesService.listForParents(classId))
                byId.set(notice.id, notice);
        }
        return [...byId.values()];
    },
