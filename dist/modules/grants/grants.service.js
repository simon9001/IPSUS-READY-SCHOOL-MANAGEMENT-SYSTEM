...
export const grantsService = {
    ...
    createType: (input) => grantsRepository.createType(input),
    ...
    async getDisbursementById(id) {
        ...
        if (!disbursement)
            throw new NotFoundError(`Grant disbursement ${id} not found`);
        return disbursement;
    },
    /**
    ...
    async recordDisbursement(input) {
        ...
        if (!grantType)
            throw new NotFoundError(`Grant type ${input.grantTypeId} not found`);
