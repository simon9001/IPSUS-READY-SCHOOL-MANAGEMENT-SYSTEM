...
export function createNotificationsService(provider = consoleNotificationProvider) {
    return {
        ...
        createTemplate: (input) => notificationsRepository.createTemplate(input),
        listByRecipient: (recipientUserId) => notificationsRepository.findByRecipient(recipientUserId),
        async send(input) {
            ...
            if (input.templateCode) {
                ...
                if (!template)
                    throw new NotFoundError(`Notification template "${input.templateCode}" not found`);
                if (!template.isActive)
                    throw new ValidationError(`Notification template "${input.templateCode}" is inactive`);
            ...
            if (!channel || !body)
                throw new ValidationError('Unable to resolve channel/body for this notification');
