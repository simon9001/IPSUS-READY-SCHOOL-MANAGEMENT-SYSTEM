...
export const feesService = {
    ...
    async getStructureById(id) {
        ...
        if (!structure)
            throw new NotFoundError(`Fee structure ${id} not found`);
    ...
    listInvoicesByStudent: (studentId) => feesRepository.findInvoicesByStudent(studentId),
    async createInvoice(input) {
        const structureItems = await feesRepository.findStructureItems(input.feeStructureId);
        if (structureItems.length === 0)
            throw new NotFoundError(`Fee structure ${input.feeStructureId} has no items`);
        ...
        for (const item of items)
            fundTotals.set(item.fundId, (fundTotals.get(item.fundId) ?? 0) + Number(item.amount));
        const lines = [
    ...
    async recordPayment(input) {
        ...
        if (invoice.status === 'paid')
            throw new ConflictError(`Invoice ${input.invoiceId} is already fully paid`);
        ...
        if (allocations.length === 0)
            throw new ValidationError('Invoice has no outstanding balance to allocate this payment against');
        ...
        await feesRepository.createAllocations(allocations.map((a) => ({ paymentId: payment.id, invoiceItemId: a.invoiceItemId, amountAllocated: a.amount.toFixed(2) })));
        const fundTotals = new Map();
        for (const a of allocations)
            fundTotals.set(a.fundId, (fundTotals.get(a.fundId) ?? 0) + a.amount);
        const lines = [...fundTotals.entries()].flatMap(([fundId, amount]) => [
        ...
        const totalAllocatedSoFar = await Promise.all(items.map((item) => feesRepository.allocatedForItem(item.id))).then((amounts) => amounts.reduce((sum, a) => sum + a, 0));
    ...
    listPaymentsByStudent: (studentId) => feesRepository.findPaymentsByStudent(studentId),
};
