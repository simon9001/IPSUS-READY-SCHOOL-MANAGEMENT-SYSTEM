...
import { journalEntries, students, admissions, feeInvoices, feePayments, budgets, purchaseRequisitions, purchaseOrders, suppliers, teachers, subjects, assets, inventoryItems, } from '../../db/schema/index.js';
    ...
    async search(query, user) {
        ...
        if (permissions.includes('ledger.journal.view')) {
            const jeRows = await db
                ...
                .from(journalEntries)
                .where(or(ilike(journalEntries.entryNo, pattern), ilike(journalEntries.description, pattern), ilike(journalEntries.sourceReference, pattern)))
                .limit(5);
        ...
        if (permissions.includes('budget.view')) {
            const budgetRows = await db
                ...
                .from(budgets)
                .where(or(ilike(budgets.name, pattern), ilike(sql `CAST(${budgets.fiscalYear} AS TEXT)`, pattern)))
                .limit(4);
        ...
        if (permissions.includes('procurement.view')) {
            const reqRows = await db
                ...
                .where(or(ilike(purchaseRequisitions.requisitionNo, pattern), ilike(purchaseRequisitions.department, pattern)))
                .limit(4);
            ...
            const suppRows = await db
                ...
                .from(suppliers)
                .where(or(ilike(suppliers.name, pattern), ilike(suppliers.kraPin, pattern), ilike(suppliers.contactPerson, pattern)))
                .limit(4);
        ...
        if (permissions.includes('students.view')) {
            const studentRows = await db
                ...
                .from(students)
                .where(or(ilike(students.firstName, pattern), ilike(students.lastName, pattern), ilike(students.admissionNo, pattern), ilike(students.nemisUpi, pattern)))
                .limit(5);
        ...
        if (permissions.includes('admissions.view')) {
            const admRows = await db
                ...
                .from(admissions)
                .where(or(ilike(admissions.applicationNo, pattern), ilike(admissions.firstName, pattern), ilike(admissions.lastName, pattern), ilike(admissions.nemisUpi, pattern)))
                .limit(4);
        ...
        if (permissions.includes('teachers.view')) {
            const teacherRows = await db
                ...
                .from(teachers)
                .where(or(ilike(teachers.fullName, pattern), ilike(teachers.staffNo, pattern), ilike(teachers.tscNumber, pattern), ilike(teachers.email, pattern)))
                .limit(4);
        ...
        if (permissions.includes('assets.view')) {
            const assetRows = await db
                ...
                .from(assets)
                .where(or(ilike(assets.name, pattern), ilike(assets.assetTag, pattern), ilike(assets.description, pattern)))
                .limit(4);
        ...
        if (permissions.includes('inventory.view')) {
            const invItemRows = await db
                ...
                .from(inventoryItems)
                .where(or(ilike(inventoryItems.name, pattern), ilike(inventoryItems.itemCode, pattern), ilike(inventoryItems.category, pattern)))
                .limit(4);
