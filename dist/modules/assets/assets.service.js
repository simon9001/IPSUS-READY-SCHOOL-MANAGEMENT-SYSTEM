...
export const assetsService = {
    ...
    createCategory: (input) => assetsRepository.createCategory(input),
    ...
    async acquire(input) {
        ...
        if (!category)
            throw new NotFoundError(`Asset category ${input.categoryId} not found`);
    ...
    async runDepreciation(input) {
        ...
        for (const asset of inUseAssets) {
            ...
            if (!category || category.depreciationMethod !== 'straight_line')
                continue;
    ...
    async dispose(assetId, input) {
        const asset = await assetsRepository.findById(assetId);
        if (!asset)
            throw new NotFoundError(`Asset ${assetId} not found`);
        if (asset.status !== 'in_use')
            throw new ConflictError(`Asset ${assetId} is not in use`);
        const category = await assetsRepository.findCategoryById(asset.categoryId);
        if (!category)
            throw new NotFoundError(`Asset category ${asset.categoryId} not found`);
        ...
        if (gainLoss > 0)
            lines.push({ accountId: input.gainLossAccountId, fundId: asset.fundId, credit: gainLoss });
        if (gainLoss < 0)
            lines.push({ accountId: input.gainLossAccountId, fundId: asset.fundId, debit: -gainLoss });
        const entry = await journalService.postSystemEntry({
            periodId: input.periodId,
