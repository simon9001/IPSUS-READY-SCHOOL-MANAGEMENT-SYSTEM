...
async function compileNemisEnrollment(periodId) {
    ...
    const newAdmissionsInPeriod = allAdmissions.filter((a) => a.status === 'enrolled' && a.enrolledAt && a.enrolledAt.toISOString().slice(0, 10) >= period.startDate && a.enrolledAt.toISOString().slice(0, 10) <= period.endDate).length;
...
async function compileTscStaffing(periodId) {
 ...
 */
async function compileMoeCapitation(periodId) {
    const period = await periodsRepository.findById(periodId);
    if (!period)
        throw new NotFoundError(`Fiscal period ${periodId} not found`);
...
export const complianceService = {
    ...
    async getById(id) {
        ...
        if (!report)
            throw new NotFoundError(`Compliance report ${id} not found`);
        return report;
    },
    ...
    async submit(id, referenceNumber, submittedBy) {
        const report = await this.getById(id);
        if (report.status === 'submitted')
            throw new ConflictError(`Report ${id} is already submitted`);
